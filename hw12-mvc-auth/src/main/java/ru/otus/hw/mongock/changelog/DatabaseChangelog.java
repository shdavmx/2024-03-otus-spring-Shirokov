...
public class DatabaseChangelog {
    ...
    private final List<LibraryRole> testRoles = List.of(
            new LibraryRole("1", "ROLE_LIBROLE", null)
    );

    private final List<LibraryUser> testUsers = List.of(
            new LibraryUser("1", "admin", "admin", testRoles)
    );
    ...
    @ChangeSet(order = "006", id = "insertRoles", author = "dshirokov")
    public void insertLibraryRoles(LibraryRoleRepository roleRepository) {
        roleRepository.saveAll(testRoles);
    }

    @ChangeSet(order = "007", id = "insertUser", author = "dshirokov")
    public void insertLibraryUsers(LibraryUserRepository userRepository) {
        BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
        for (LibraryUser user : testUsers) {
